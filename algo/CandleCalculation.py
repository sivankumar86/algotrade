...
def roc(dfp):
    dfp['prevclose']=dfp['close'].shift(1)
    dfp['roc'] = round(((dfp['close'] - dfp['close'].shift(5)) / dfp['close'].shift(5)) * 100)
    dfp['roctradel'] = dfp.apply(lambda row: row['close']> row['prevclose'] and row['roc'] < -2, axis=1)
    dfp['roctrades'] = dfp.apply(lambda row: row['close'] < row['prevclose'] and row['roc'] > 2, axis=1)
    return dfp


def hammer(dfp):
    dfp['hammer'] = dfp.apply(
        lambda row: row['roc'] < -1 and row['close'] - row['low'] > (2 * (row['open'] - row['close'])) and row['high'] - row['open'] < (
...
def shootingStar(dfp):
    dfp['shooting'] = dfp.apply(
        lambda row: row['roc'] > 1 and row['high'] - row['open'] > (2 * (row['open'] - row['close'])) and row['close'] - row['low'] < (
